import Styles from './gallery-products.module.scss';
...
export const GalleryProducts = ({ photos = [] }: TProps) => {
  ...
  return (
    <div className={Styles.sliderProducts}>
      <div className={Styles.itemProducts}>
        <img className={Styles.thumbnailProducts} src={showPhoto.src} alt={showPhoto.alt} onClick={() => setIsOpenModal(true)} />
      </div>

      {photos.length > 1 && (
        <>
          <a className={Styles.previousProducts} onClick={previousSlide}>
            &#10094;
          </a>
          <a className={Styles.nextProducts} onClick={nextSlide}>
            &#10095;
          </a>
        </>
      )}
